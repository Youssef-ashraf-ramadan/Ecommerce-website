...
const section = document.querySelector('section.product-info');
const productsKind = section.dataset.kind;
const recommendedDom = document.querySelector('.recommended-products--js');
...
// display products
class UI {
  renderProducts(products, whereToAppend) {
    let result = '';
    for (let i = 0; i <= 3; i += 1) {
  ...
  renderProductsTDom(productsData) {
    ...
    this.renderProducts(productsData, recommendedDom);
  }
